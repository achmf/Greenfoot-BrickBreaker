...
public class Level2World extends World {
    ...
    static GreenfootSound gfs = new GreenfootSound("2background-music.mp3");

    public Level2World() {
        super(800, 600, 1);
        setBackground("lv2bg.png");
        prepare();
        gfs.setVolume(30);
        gfs.play();
        WinScreen.gfs.stop();
    ...
    private void prepare() {
        ...
        for (int j = 0; j < bricksPerRow; j++) {
            Block brick = new TwoHitBlock(brickWidth); // Use TwoHitBlock for Level2
        ...
        ball.setXSpeed(5); // Set the horizontal speed to 5
        ball.setYSpeed(-5); // Set the vertical speed to -5
    }
    ...
    public void act() {
        if (noBlocksRemaining()) {
            Greenfoot.setWorld(new WinScreen2()); // Transition to the win screen
        }
    }
}
